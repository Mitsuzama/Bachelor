...
namespace InfoShow
{
	[ExecuteInEditMode] // updates the UI elements in real time
	public class GraphicEffects : MonoBehaviour
	{
		...
		protected virtual void Start()
		{
			...
			foreach (var graphic in uiElementsToApplyTo)
			{
				...
				if (material == null)
				{
					Debug.LogError($"{nameof(GraphicEffects)}: skipping target without material {graphic.name}.{graphic.GetType().Name}");
					continue;
				}
